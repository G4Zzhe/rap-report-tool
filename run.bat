...
REM 检查 config.local.yaml 是否存在
if not exist "config.local.yaml" (
    echo [提示] 未找到 config.local.yaml。
    echo        如需 AI 文案，请复制 config.yaml 为 config.local.yaml 并填写 AI 信息。
    echo        如不需要 AI，请使用 run_no_ai.bat。
    echo.
)
...
if "%CHOICE%"=="2" goto :custom_date
if "%CHOICE%"=="1" goto :default_date
...
:custom_date
echo.
set /p START_DATE=请输入开始日期（如 2026-07-16）：
set /p END_DATE=请输入结束日期（如 2026-07-31）：
